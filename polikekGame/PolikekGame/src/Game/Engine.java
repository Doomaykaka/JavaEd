...
public class Engine{
	...
	public void init() {
		...
		JLabel background = new JLabel(getBackground());
	    background.setLayout(new FlowLayout());
	    background.setVisible(true);
	    background.setSize(1024, 1024);
		...
		JButton saveGame = new JButton("Save button");
		saveGame.setText("Save game");
		saveGame.setBounds(442, 230, 140, 30);
		...
		JButton loadGame = new JButton("Load button");
		loadGame.setText("Load game");
		loadGame.setBounds(442, 360, 140, 30);
	...
	void saveGame() {
		try(FileWriter writer = new FileWriter("save.dat", false))
        {
			if(map!=null) {
				writer.write(map.getNextMap());         
	            writer.flush();
			}
        }
	...
	void loadGame() {
		...
		try(Scanner reader = new Scanner(save))
        {
			if(reader.hasNextLine()) {
				...
				linkMap.setNextMap(mapName);
				this.setMap(linkMap);
				this.needNextMap=true;
			}
        }catch(Exception ex) {
        	ex.printStackTrace();
        }   
	...
	ImageIcon getBackground() {
		...
		Texture addedTexture = new Texture("BACKGROUND","BACKGROUND.png");
		background = addedTexture.getTexture();
		return background;
	}
